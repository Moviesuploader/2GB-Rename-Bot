...
async def video_info_handler(c: Client, m: Message):
    try:
        if filesystem_free() < 5000000000:
            raise Exception("Insufficient server space.")
        ...
        if not file_type.mime_type.startswith("video/"):
            raise Exception("This is not a Video!")
        ...
        try:
            ...
            raise Exception(f"Error downloading video: {download_error}")
        ...
        try:
            ...
            raise Exception(f"Error fetching media metadata: {metadata_error}")

        if not output:
            await rm_dir(root_dl_loc)
            raise Exception("Can't fetch media info!")
        ...
        try:
            ...
            try:
                ...
                await rm_dir(root_dl_loc)
                raise Exception(f"Error processing video: {processing_error}")

            await editable.edit("Renamed Successfully!")

        except Exception as main_error:
            # Handle the main exception (e.g., log, notify, or reply to the user)
            await m.reply_text(f"Error: {main_error}", True)
            return
        ...
        # Handle any final exception here (e.g., log, notify, or reply to the user)
